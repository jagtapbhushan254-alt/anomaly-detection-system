...
def call_api(txn_dict):
    fields = ["amount","merchant_category","hour_of_day","day_of_week",
              "transaction_count_1h","avg_amount_7d","distance_from_home_km"]
...
with ch:
    ...
    if st.session_state.transactions:
        ...
        fig = go.Figure()
        for flag, color, sym, name in [(False,"#00cc44","circle","Normal"),(True,"#ff4444","x","Anomaly")]:
        ...
        fig.update_layout(height=300, paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                         ...
                         xaxis=dict(showgrid=False), yaxis=dict(title="Amount ($)", gridcolor="#333"))
...
if len(st.session_state.transactions) > 10:
    ...
    d1, d2 = st.columns(2)
    with d1:
        fig2 = px.histogram(df_all, x="anomaly_score", color="is_anomaly", nbins=30,
            title="Isolation Forest Scores", template="plotly_dark",
            color_discrete_map={False:"#00cc44", True:"#ff4444"})
        fig2.update_layout(height=250, margin=dict(l=0,r=0,t=30,b=0))
        st.plotly_chart(fig2, use_container_width=True)
    with d2:
        fig3 = px.histogram(df_all, x="reconstruction_error", color="is_anomaly", nbins=30,
            title="Autoencoder Reconstruction Error", template="plotly_dark",
            color_discrete_map={False:"#00cc44", True:"#ff4444"})
...
if st.session_state.running:
    ...
    if result:
        row = {"amount":txn.amount,"merchant_category":txn.merchant_category,
