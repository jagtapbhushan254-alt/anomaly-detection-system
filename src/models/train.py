...
def train_all(n_samples: int = 10000, fraud_rate: float = 0.02):
    ...
    logger.info(f"Step 1/4: Generating {n_samples} transactions (fraud_rate={fraud_rate:.1%})")
    ...
    print("\n  SAMPLE PREDICTIONS:")
    print(f"  {'Type':<10} {'IF Score':<12} {'AE Error':<12} {'IF Flag':<10} {'AE Flag'}")
    print("  " + "-"*55)
    ...
    for _, row in normal_samples.iterrows():
        ...
        print(f"  {'NORMAL':<10} {if_pred['anomaly_score']:<12.4f} "
    ...
    for _, row in fraud_samples.iterrows():
        ...
        if_pred = if_model.predict(features)
        ae_pred = ae_model.predict(features)
        print(f"  {'FRAUD':<10} {if_pred['anomaly_score']:<12.4f} "
              f"{ae_pred['reconstruction_error']:<12.6f} "
